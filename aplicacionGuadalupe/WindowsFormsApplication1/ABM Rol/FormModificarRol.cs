       ...
       private void btnDesasignar_Click(object sender, EventArgs e)
       {
        ...
        #region metodosInterfase
           public void ejecutarSQL()
           {
               try //hacer esto bien
               {
                   SQL.ejecutar_SP("EXEC TPGDD.SP_MODIFICAR_ROLES_OK " + id + ", '" + txtNombre.getValor() + "','" + chkHabilitado.getValor() + "'");
                   foreach (var item in listaAsignadas.Items) 
                    {
                        SQL.ejecutar_SP("EXEC TPGDD.SP_AGREGAR_FUNCIONALIDAD_ROL_OK '" + txtNombre.getValor() + "', '" + item.ToString() + "'");
                    }
